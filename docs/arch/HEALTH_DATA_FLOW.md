...
- Routine create/edit: Add flow validates input before repository persistence.
...
- Timing/adherence formulas: `DoseTimingPolicy` owns due-soon, missed, late and completion formulas.
- History/coach UI consumes persisted records and derived policies; it does not own formulas or persistence.
- Import: `ImportBackupUseCase` previews duplicate/orphan/remap impact before `importBackupAtomic`; rollback is owned by the persistence boundary.
...
- iOS uses `LocalDayCodec`; it preserves the literal calendar day in the supplied local calendar/time zone and does not round-trip through UTC.
...
1. Decode supported schema/version and verify integrity when a manifest is present.

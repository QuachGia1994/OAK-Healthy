...
enum ClientScopedStore {
    ...
    static func hasSupplementChanges(
        ...
        var descriptor = FetchDescriptor<UserSupplement>(predicate: #Predicate {
            $0.client?.id == clientId &&
            ($0.updatedAtEpochMs > lastSyncEpochMs || ($0.deletedAtEpochMs ?? 0) > lastSyncEpochMs)
        })
    ...
    static func hasHistoryChanges(
        ...
        var descriptor = FetchDescriptor<IntakeRecord>(predicate: #Predicate {
